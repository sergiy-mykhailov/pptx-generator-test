...
function genSlide01(pptx) {
  ...
  const objOpts3 = { x: 0.5, y: 3.6, h: 0.3, margin: 0, fontSize: 18, fontFace: 'Arial', color: '0088CC' }
...
function genSlide02(pptx) {
  ...
  slide.addTable([[{ text: 'Table Examples 2', options: BASE_TEXT_OPTS_L }, BASE_TEXT_OPTS_R]], {
    x: '4%',
    y: '2%',
    w: '95%',
    h: '4%',
  }) // QA: this table's x,y,w,h all using %
  slide.addNotes('API Docs: https://gitbrent.github.io/PptxGenJS/docs/api-tables.html')

  // DEMO: Rowspans/Colspans
  const optsSub = JSON.parse(JSON.stringify(BASE_OPTS_SUBTITLE))
...
function genSlide03(pptx) {
  ...
  const optsRowspan2 = { rowspan: 2, fill: { color: '99FFCC' } }
  const optsRowspan3 = { rowspan: 3, fill: { color: '99FFCC' } }
  const optsRowspan4 = { rowspan: 4, fill: { color: '99FFCC' } }
  const optsRowspan5 = { rowspan: 5, fill: { color: '99FFCC' } }
  const optsColspan2 = { colspan: 2, fill: { color: '9999FF' } }
  const optsColspan3 = { colspan: 3, fill: { color: '9999FF' } }
  const optsColspan4 = { colspan: 4, fill: { color: '9999FF' } }
  const optsColspan5 = { colspan: 5, fill: { color: '9999FF' } }
  ...
  const arrTabRows5 = [
    ...
    [
      ...
      'E5',
      'F5',
      { text: 'G5 -> H5', options: { colspan: 2, fill: { color: '9999FF' } } },
      'J5',
      'K5',
      'L5',
    ],
...
function genSlide04(pptx) {
  ...
  slide.addTable([['margin:[0, 0, 0, 0.3]']], {
    x: 2.5,
    y: 1.1,
    margin: [0, 0, 0, 0.3],
    w: 2.0,
  ...
  slide.addTable([['margin:[0.6, 0.05, 0.05, 0.3]']], {
    x: 7.1,
    y: 1.1,
  ...
  slide.addTable([['margin:[0.45, 0.05, 0.05, 0.45]']], {
    x: 10.1,
    y: 1.1,
  ...
  slide.addTable([[{ text: 'text-obj margin:0', options: { margin: 0 } }]], {
    x: 4.0,
    y: 1.9,
    w: 2,
    fill: { color: 'f2f9fc' },
  ...
  slide.addTable(arrTextObjects, {
    x: 0.5,
    y: 2.7,
    w: 12.25,
    margin: 8,
    fill: { color: 'F1F1F1' },
    border: { pt: 1, color: '696969' },
  }) // DEPRECATED: `margin` in points
  ...
  const arrBorder2 = [{ type: 'dash', color: 'ff0000', pt: 2 }, null, { type: 'dash', color: '0000ff', pt: 5 }, null]
...
function genSlide06(pptx) {
  ...
  const arrCell3 = [
    {
      text: 'google',
      options: { bullet: true, color: pptx.colors.ACCENT1, hyperlink: { url: 'https://www.google.com' } },
    },
  ]

  const arrCell4 = [{ text: 'Numbers\nNumbers\nNumbers', options: { color: '0088cc', bullet: { type: 'number' } } }]
  slide.addTable(
    [
      [
        { text: arrCell1 },
...
function genSlide07(pptx) {
  ...
  const arrRowsHead2 = [
    [
      {
        text: 'Title Header',
        options: { fill: '0088cc', color: 'ffffff', align: 'center', bold: true, colspan: 3, colW: 4 },
      },
    ],
  ]
  {
    ...
    arrRows.forEach((row, idx) => {
      if (idx < 6) {
        arrRowsHead1.push(row)
      }
    })
    arrRows.forEach((row, idx) => {
      if (idx < 6) {
        arrRowsHead2.push(row)
      }
    })
  }
  ...
  // EX-3: "Master Page with Auto-Paging"
  {
    ...
    slide.newAutoPagedSlides.forEach((slide) =>
      slide.addText('Auto-Paging table continued...', { placeholder: 'footer' }),
    )
  }
  ...
  // EX-6: "Start at `{ y: 4.0 }`, subsequent slides start at `{ autoPageSlideStartY: 1.5 }`"
  {
    ...
    slide.addText(
      [
        { text: 'Table Examples: ', options: DEMO_TITLE_TEXT },
        {
          text: 'Start at `{ y: 4.0 }`, subsequent slides start at `{ autoPageSlideStartY: 1.5 }`',
          options: DEMO_TITLE_OPTS,
        },
      ],
      { x: 3.0, y: 0.75, w: '75%', h: 0.5 },
    )
...
function genSlide09(pptx) {
  ...
  for (let rowIdx = 0; rowIdx < 9; rowIdx++) {
    const col3Lines = [{ text: 'Complex-Title', options: { bold: true, color: 'FF0000', breakLine: true } }]
